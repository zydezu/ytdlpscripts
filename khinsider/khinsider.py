...
if __name__ == "__main__":
        ...
        for module in modules:
            ...
            try:
                ...
                if verbose:
                    ...
                    print(
                        "You can also try to install the package manually "
    ...
    if needed:
        if moduleExists("pip"):
            ...
            print(
                "Get it from https://pip.readthedocs.org/en/latest/installing.html",
                file=sys.stderr,
            )
            sys.exit(1)
...
class Song:
    ...
    * files: A list of the song's files - there may be several if the song
    ...
    def files(self):
        ...
        anchors = self._soup(
            "a", href=re.compile(r"^https?://[^/]+/(?:soundtracks|ost)/.+$")
        )
...
def soundtracksInSearchTable(table):
    ...
    soundtrackParams = [
        (a["href"].split("/")[-1], a.get_text(strip=True)) for a in anchors
    ]
...
def printSearchResults(searchResults, file=sys.stdout):
    ...
    for heading, soundtracks in zip(
        ("Album title results:", "Song name results:"), searchResults
    ):
        if soundtracks:
            if hasPreviousList:
                s += "\n"
...
if __name__ == "__main__":
    ...
    class KindArgumentParser(argparse.ArgumentParser):
        def error(self, message):
            ...
            print(
                'For detailed help and more options, run "{} --help".'.format(
                    SCRIPT_NAME
                ),
                file=sys.stderr,
            )
    ...
    def doIt():  # Only in a function to be able to stop after errors, really.
        parser = KindArgumentParser(
            description="Download entire soundtracks from KHInsider.\n\n"
            "Examples:\n"
        ...
        parser.add_argument(
            "trailingArguments", nargs=argparse.REMAINDER, help=argparse.SUPPRESS
        )
        ...
        parser.add_argument(
            ...
            '(for example, "flac, ogg, mp3": download FLAC if available, otherwise OGG, otherwise MP3).',
        )
        parser.add_argument(
            "-s",
            "--search",
        ...
        try:
            if onlySearch:
                ...
                try:
                    success = download(
                        soundtrack, outPath, formatOrder=formatOrder, verbose=True
                    )
                    if not success:
                    ...
                    if searchResults:  # aww yeah we gon' do some searchin'
                        ...
                        print(
                            'A search for "{}" could not be performed either. '
                            "It may be too short.".format(searchTerm),
                            file=sys.stderr,
                        )
                    ...
                    s = (
                        ...
                        'The soundtrack "{}" is only available in the '
                    ).format("" if len(formatOrder) == 1 else "s", soundtrack)
                    ...
                    if len(formats) == 1:
                        ...
                        s += '{}{} and "{}" formats.'.format(
                            ", ".join(
                                '"{}"'.format(extension) for extension in formats[:-1]
                            ),
            ...
            print(file=sys.stderr)
            print("An unexpected error occurred! " + REPORT_STR, file=sys.stderr)
