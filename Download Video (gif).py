...
command = f'yt-dlp {quality} "{link}" --add-metadata --write-subs --embed-subs --embed-thumbnail --cookies cookies.txt -o "{output_path}"'
...
if video_file:
    ...
    subprocess.run(
        f'ffmpeg -y -i "{video_file}" -vf palettegen palette.png', shell=True
    )
    subprocess.run(
        f'ffmpeg -i "{video_file}" -i palette.png -filter_complex "[0:v][1:v]paletteuse" "{gif_file}"',
        shell=True,
    )
